...
public class ArrayUtil {
	...
	public static <K, V> Map<K, V> zip(K[] keys, V[] values, boolean isOrder) {
		...
		Map<K, V> map = isOrder ? new LinkedHashMap<>(initialCapacity) : new HashMap<>(initialCapacity);
		...
		if (isArray(array)) {
			...
			if (componentType.isPrimitive()) {
				...
				switch (componentTypeName) {
					...
					default:
						throw new RuntimeException(StrUtil.format("Unknown primitive type: [{}]", componentTypeName));
				}
			} else {
				return join((Object[]) array, conjunction);
			}
		}
		throw new RuntimeException(StrUtil.format("[{}] is not a Array!", array.getClass()));
	}
